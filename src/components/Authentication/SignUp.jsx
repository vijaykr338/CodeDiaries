...
function SignUp() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      handleTextColorChange(2);  // Set success message color
      setParagraphText(data.message || 'Registration successful');  // Show success message
      console.log('Success:', data);
  
    } catch (error) {
      // Display the error message from the server or a generic one
      handleTextColorChange(0);  // Set error message color
      setParagraphText(error.message || 'Registration failed. Try again later.');
