...
function Edit() {
   ...
   const OnSubmit = async (data) => {
      ...
      let r = await fetch('http://localhost:3000/posts/create-post', {method: "POST",  headers: {
   ...
   return (
      <>
      <div className="overflow-x-hidden w-screen bg-black"><Header /></div>
         <div className="flex justify-center flex-shrink-0 bg-black h-full w-full">
            <div className="flex w-11/12 sm:w-4/5 bg-white my-10 px-10 h-full py-6 flex-col rounded-lg">
               ...
               <div className="flex flex-col flex-shrink-0 sm:w-3/5 justify-start gap-6 my-7 sm:text-2xl">
                  <form className="flex flex-col gap-9" onSubmit={handleSubmit(OnSubmit)} action="/createpost">
                     <input className="border-2 border-black rounded-md px-6 py-3  placeholder:text-black"
                     ...
                     <input onClick={notify} type="submit" className="border-2 border-black bg-black font-bold text-white py-2 px-3  rounded-full hover:shadow-xl shadow-cyan-300"/>
                     <ToastContainer
                        position="top-right"
                        autoClose={5000}
                        hideProgressBar={false}
